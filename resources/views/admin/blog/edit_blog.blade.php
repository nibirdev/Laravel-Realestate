            ...
            <div class="col-md-12 col-xl-12 middle-wrapper">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form class="forms-sample" method="POST" action="{{route('blog.post.update',$blog->id)}}" enctype="multipart/form-data">
                            @csrf
                            <div class="mb-3">
                                <label for="state_name" class="form-label">Title</label>
                                <input type="text" class="form-control" id="state_name" value="{{$blog->title}}" name="title" value="{{old('state_name')}}" autocomplete="off">

                            </div>
                            <div class="mb-3">
                                <label for="state_name" class="form-label">Short Description</label>
                                <textarea class="form-control" id="exampleFormControlTextarea1" rows="5" value="" name="short_des">{{$blog->short_des}}</textarea>

                            </div>
                            <div class="mb-3">
                                <label for="state_name" class="form-label">Long Description</label>
                                <textarea class="form-control" name="long_des" id="tinymceExample" rows="10">{{$blog->long_des}}</textarea>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="cat_id" class="form-select" id="exampleFormControlSelect1">
                                    ...
                                    @foreach($cat as $blogs)
                                    <option value="{{$blogs->id}}" {{$blog->cat_id==$blogs->id?'selected':''}}>{{$blogs->cat_name}}</option>
                                    @endforeach
                                </select>
                            </div>

                            <div class="mb-3">
                                <label for="state_name" class="form-label" value="{{$blog->tag}}">Tags</label>
